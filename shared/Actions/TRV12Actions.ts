...
import { intercitySequence } from '../../constants'
import { search } from '../../utils/TRV12/search'
// import { compareSTDwithArea } from 'utils'
...
export function validateLogsForTrv12(data: any, flow: string, version: string) {
  ...
  try {
    if (data[intercitySequence.SEARCH]) {
      const searchResp = search(data[intercitySequence.SEARCH], msgIdSet)
